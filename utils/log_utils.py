...
def log_action(user_id=None, action=None, target_table=None, target_id=None, description=None):
    if not user_id and not session.get('user_id'):
        logging.warning('로그인 되지 않은 사용자 행동 기록 시도 (무시)')
        return
    if not user_id:
    ...
    if user_id is not None:
        with get_db() as conn:
            with conn.cursor() as cursor:
                ...
                if is_admin == 1:
                    log_admin_action(user_id, action, target_table, target_id, description)
                    return
                else:
                    log_user_action(user_id, action, target_table, target_id, description)
                    return
    else:
        logging.warning('로그인 되지 않은 사용자입니다. 사용자 행동 기록을 건너뜁니다.')
        return
